...
7b. **Legacy Example Program PRD or BRD: add a new world carry-over row (standing rule, the owner 23 Sep 2026).** Every PRD and BRD written for the legacy Example Program store also gets a row under epic **AFM-29** in the Example Program Migration Backlog, so the feature is rebuilt on the E-commerce Solution. It is tracked separately: own wave `Carry-over`, label `legacy-carryover`, never merged into an existing cutover row. Where a cutover row overlaps, name it under Related in the description. Steps: add the feature to `ITEMS` in `Clients/Work/Example Program/migration_backlog/add_carryover_afm29.py`, run it, then `push_afm29.py` (dry run first, `--approved` once the owner approves the sheet write). Work-tree home: `nw-afcarry`. This applies to client BRDs too, even though they stay out of the MPL.
