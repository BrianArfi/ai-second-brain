...
- **A dispatch counts only once the app has a chat for it.** On 21 Sep 2026 two requests left the queue without becoming a chat, and because an open conversation is never re-dispatched, Teammate's question was lost until the owner went looking. Every `run` now checks the app's own `branches.json` and transcript folder for each conversation older than 15 minutes, and takes back the ones with no chat behind them. Only the machine running the app performs the check; every other host leaves the record alone. Two failed revivals and it stops and reports instead, as a `NO CHAT:` line in `status`. Regression test: `python3 tests/test_reply_router_dead_dispatch.py`.
