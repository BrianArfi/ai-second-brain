...
argument-hint: "on | off | status | list | run | close --all-answered | on --source gmail"
---
...
- `run` is the dispatch pass itself. The ASB app routine `reply-router` calls `/autodraft run` every 15 minutes, 07:00-23:59 WIB, on whichever machine has the app open. Add `--dry-run` to see what would dispatch without opening sessions.
- A toggle takes effect on the next routine tick, within 15 minutes, with the app open.
