...
- **Same on/off switch as reply-router.** `draft` reads `journal/state/automation_config.json` -> `auto_reply_drafts.enabled` (and `.sources.slack`) before calling agy-bridge - the exact config the ASB app's "Auto reply drafts" toggle writes. Off means `draft` prints a one-line notice and does nothing (no agy-bridge call, no state write). This is the same toggle the reply-router ticker checks, so one switch now covers both the live ticker and this digest. `report` is unaffected - it only re-renders whatever was already drafted, no LLM call either way.
